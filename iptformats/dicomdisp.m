...
%   DICOMDISP(FILENAME) reads the metadata from the compliant
...
readPixels = false;
rawAttributes = dicomparse(fileDetails.name, ...
                           fileDetails.bytes, ...
                           getEndian(), ...
                           readPixels, ...
                           dicomdict('get'), ...
...
function fileDetails = getFileDetails(filename)

fileDetails = dir(filename);
...
fprintf(buildFormatString(), ...
        ...
        dicomlookup_actions(attr.Group, attr.Element, dictionary), ...
        data);
